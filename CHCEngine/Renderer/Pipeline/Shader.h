...
public:
  ...
  uint32_t getConstantBufferSizeByName(const std::string &name) const;
...
private:
  ...
  std::unordered_map<std::string, BindFormat> resource_bind_table_;
  std::unordered_set<std::string> root_constant_names_;
...
public:
  ...
  ShaderSet(const std::unordered_map<ShaderType, Shader> &shader_set,
            const std::vector<std::string> &root_constants_names);
  ShaderSet(const std::vector<Shader> &shader_set,
            const std::vector<std::string> &root_constants_names = {});
  ...
  const BindFormat &getBindFormat(std::string name) const {
    if (!resource_bind_table_.count(name)) {
      throw std::exception(
          (std::string("Can't find ") + name + std::string(" in Shader"))
              .c_str());
    }
    return resource_bind_table_.at(name);
  }
